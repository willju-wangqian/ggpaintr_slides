...
- **Exploration tool**
  - visualize feature relationships quickly 
  - enable diagnostic workflow 
- **Shiny dashboard**
  - present a **finished** analysis in a Shiny app, with pre-defined and chosen controls exposed to users
...
`shiny::runApp("demos/01-iris-explore.R")`
...
![](images/ggpcp_published_figure.png){height="360" fig-align="center"}

. . .

`shiny::runApp("demos/02-ggpcp-dashboard.R")`
...
- each new control requires both `UI` and `server` code 
:::


## Reproduce the same app with ggpaintr 

App layout:

![](images/annotated/annotated_iris_app.png){width="80%" fig-align="center"}
...
ptr_app(                                 # make an app based on this formula
  ggplot(iris, aes(
    x = ppVar(Sepal.Length),             # a picker for x-axis, default Sepal.Length
    y = ppVar(Sepal.Width),              # a picker for y-axis, default Sepal.Width
    color = ppVar(Species))) +           # a picker for color, default Species
    geom_point()
)
```
...
- Lazy evaluation and metaprogramming in R
...
- Placeholders: five default placeholders and three roles
- Embellishment: what and why
- Interactive masking
...
- Multiple placeholders sharing the same widget (linking placeholders)
...
- code panel
- `spec` (specification, current user's input widget values)

## The code panel {.smaller}

`ggpaintr` returns **runnable `ggplot2` code for the displayed plot** 
...
- The app **reopens in that exact state** — values in `spec` are used as the default values
...
serves a user constructing **their own Shiny app**. 
...
**vs. esquisse** (closest neighbor): `esquisse` offers a chart-authoring menu and shows high level of flexibility. But still, it is not used for generating shiny apps
...
- `demos/03-iris-penguins-demo.R` — the full value/consumer showcase
...
- `demos/07-iris-penguins-pca-demo.R` – PCA on `iris` and `penguins` data
- `demos/08-ppExpr-demo.R` – `ppExpr` demonstration
...
- Reproducibility stops at what the artifacts record: 
  - the app reproduces given the **same ggpaintr version** and any **custom-placeholder registrations** (those live outside the formula text) 
  - the code panel does **not** capture package versions or transformations the calling app applies *after* ggpaintr hands the object back.
- The **denylist** on `ppExpr` is a safety check for the trusted developer/user workflow — **not** a security boundary. A deployment taking formulas from outside parties must add its own trust boundary.

## Limitations & future work

**Language models**

- Powerful large Language models can output Shiny apps directly
  - deterministic vs random
  - high quality outputs need "plan, implement, verify, (and maintain)"
- Complementary, not competing
  - `ptr_llm_primer()` / `ptr_llm_topic()` / `ptr_llm_register()`
  - teach an LLM how to use `ggpaintr`
  - whether it measurably improves LLM reliability might need future work

CRAN release, ggpaintr ebook, and more custom placeholders from community
